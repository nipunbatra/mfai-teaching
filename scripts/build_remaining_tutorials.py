...
def latex_math(text: str) -> str:
    ...
    def convert(match: re.Match[str]) -> str:
        ...
        expression = re.sub(r"\blog_", r"\\log_", expression)
...
TUTORIALS = [
    {
        ...
        "solutions": [
            "$x^Ty=4$, $||x||=3$, $||y||=sqrt(5)$, so cosine $=4/(3sqrt(5)) approx 0.596$. The projection is $(x^Ty/y^Ty)y=(4/5)y$ — the dot product measures overlap, and dividing by $y^Ty$ turns it into a coefficient along $y$.",
            "$Ae_1=(2,0)$, $Ae_2=(1,1)$; the square becomes their parallelogram. $det A=2$, so area doubles and the map is invertible.",
            "Normal equations $A^TA hat x=A^Tb$: $A^TA=[[2,1],[1,2]]$, $A^Tb=(5,6)$, hence $hat x=(4/3,7/3)$. They state exactly that the residual $b-A hat x$ is orthogonal to both columns.",
    ...
    },
    {
        ...
        "solutions": [
            "Differentiate term by term: $nabla f=(2x+y,x+4y)$, which is $(1,-3)$ at $(1,-1)$.",
            "The directional derivative is $nabla f^T u=(1,-3)^T(3,4)/5=-9/5$ — negative, so $f$ decreases in that direction.",
            "One tangent is $(3,1)$ because $(1,-3)^T(3,1)=0$.",
            "$partial_x z=2x cos(x^2+3y)$ and $partial_y z=3cos(x^2+3y)$ — the chain rule multiplies the outer $cos$ by each inner partial.",
    ...
    },
    {
        ...
        "solutions": [
            "$a=2(-3)=-6$, $b=-6+2=-4$, $L=(-4)^2=16$.",
            "$bar b=2b=-8$, $bar a=bar b=-8$, $bar x=bar b+bar a y=-8+24=16$ (the direct path through $b$ plus the path through $a$), and $bar y=bar a x=-16$.",
            ...
            "Seed has shape $1 times m$ and the VJP has shape $1 times n$ — reverse mode carries a row vector backward without ever forming the full Jacobian.",
        ],
        "cells": [
    ...
    },
    {
        ...
        "solutions": [
            ...
            "$P(X>1)=e^{-2} approx 0.135$ (the survival function $e^{-lambda x}$) and $E[X]=1/lambda=1/2$.",
            "$E[Y]=3(2)-1=5$ and $Var(Y)=3^2 Var(X)=81$ — shifting moves the mean only; scaling multiplies the variance by the square.",
    ...
    },
    {
        ...
        "solutions": [
            ...
            "Conjugacy: add heads to the first parameter and tails to the second, giving posterior Beta$(5,3)$ with mean $5/8$.",
            "The interior mode of Beta$(a,b)$ is $(a-1)/(a+b-2)$, here $2/3$; the mean $5/8$ sits closer to $1/2$ because it averages over the whole density.",
            "Both yield Beta$(5,3)$ because sufficient statistics add.",
            "Beta$(10,2)$ acts like $9$ prior heads and $1$ prior tail in the mode formula; once the real counts dwarf these fixed pseudo-counts, the likelihood dominates.",
        ],
        "cells": [
            md("""## 1 · Likelihood and posterior on one plot"""),
            code("""#| fig-alt: 'Scaled Bernoulli likelihood, Beta prior, and Beta posterior over the coin bias after three heads and one tail, with the MLE marked.'\nimport numpy as np\nimport matplotlib.pyplot as plt\nfrom scipy import stats\n\ntheta = np.linspace(.001, .999, 600)\nh, t = 3, 1\nlikelihood = theta**h * (1-theta)**t\nprior = stats.beta(2, 2).pdf(theta)\nposterior = stats.beta(2+h, 2+t).pdf(theta)\nfor y, label in [(likelihood/likelihood.max(), "likelihood (scaled)"), (prior/prior.max(), "prior"), (posterior/posterior.max(), "posterior")]:\n    plt.plot(theta, y, label=label)\nplt.axvline(h/(h+t), color="#EB811B", ls="--", label="MLE")\nplt.legend(); plt.xlabel("coin bias"); plt.grid(alpha=.15);"""),
    ...
    },
    {
        ...
        "solutions": [
            "$x_(k+1)=x_k-eta x_k=(1-eta)x_k$; convergence needs $|1-eta|<1$, that is $0<eta<2$.",
            "The Hessian is diagonal, so the coordinates decouple: $x^+=(1-eta)x$ and $y^+=(1-50 eta)y$.",
            "Stability of the steep direction needs $|1-50 eta|<1$, so $eta<2/50=0.04$; at that ceiling the flat direction contracts by only $1-eta approx 0.96$ per step.",
    ...
    },
    {
        ...
        "solutions": [
            "$x^+=x-f'/f''=2x-x^2$; $0.5 to 0.75 to 0.9375$. The error $1-x$ squares each step: $0.5, 0.25, 0.0625$.",
    ...
    },
    {
        ...
        "solutions": [
            "Stationarity gives $y=lambda$ and $x=lambda$, so $x=y$; the constraint then forces $x=y=1/2$, $lambda=1/2$, objective $1/4$.",
            "$x=y=b/2$, $V(b)=b^2/4$, and $V'(b)=b/2=lambda$ — the multiplier prices the constraint.",
            "Inactive gives infeasible $x=2$; active gives $x=1$, $lambda=2$.",
            "The unconstrained minimizer $x=.2$ is already feasible, so it is optimal with slack $.8$; complementary slackness then forces the multiplier to zero.",
            "Subtract a common threshold and clip at zero: threshold $.75$ gives $(2.25,.25,0)$, which is non-negative and sums to the budget $2.5$.",
        ],
        "cells": [
            md("""## 1 · Contours meeting a constraint"""),
            code("""#| fig-alt: 'Contours of the product x times y, the line x plus y equals one, and the constrained maximizer at one-half, one-half.'\nimport numpy as np\nimport matplotlib.pyplot as plt\n\nx=np.linspace(0,1,200); X,Y=np.meshgrid(x,x)\nplt.contour(X,Y,X*Y,levels=10); plt.plot(x,1-x,color="#EB811B",lw=3)\nplt.scatter([.5],[.5],color="#D64550"); plt.axis("equal");"""),
    ...
    },
    {
        ...
        "solutions": [
            "The feasible vertices are $(0,0),(2,0),(2,2),(1,3),(0,3)$ with values $0,6,10,9,6$; an LP optimum sits at a vertex, here $(2,2)$ with value $10$.",
            "$y<=3$ is slack, so its price is $0$; both primal variables are positive, so the two dual constraints bind, giving prices $(2,1,0)$ and value $4(2)+2(1)=10$ — equal to the primal, as strong duality promises.",
    ...
    },
    {
        ...
        "problems": [
            ...
            ("Huffman ⭐⭐", "Run the Huffman merges for weights $(40,20,15,15,10)$, listing the combined weight at each step."),
        ...
        "solutions": [
            "$H=-sum_i p_i log_2 p_i approx 2.146$ bits, just under the $log_2 5 approx 2.322$ of a uniform five-symbol source.",
            "$1/2+4/8=1$, so it exactly fills a binary tree.",
            "Merge $10+15$, $15+20$, $25+35$, then $40+60$ (ties may change codewords, not expected length). The resulting depths $(1,3,3,3,3)$ are exactly Problem 2's lengths; average length $2.2$ bits against entropy $approx 2.146$.",
            "$H(p)approx.992$, $H(p,q)approx1.283$, so KL $=H(p,q)-H(p)approx.291$ bits — the price of coding $p$ with a code built for $q$.",
            "Perplexity exponentiates in the matching base: $2^{2.5}approx5.657$ and $e^{2.5}approx12.182$.",
        ],
        "cells": [
    ...
    },
    {
        ...
        "problems": [
            ("Stationary distribution ⭐", "For $P=[[.8,.2],[.4,.6]]$ over states $(S,R)$, solve $pi=pi P$ and normalize."),
            ("Path probability ⭐", "With states ordered $(S,R)$ and initial distribution $(.6,.4)$, find $P(S,R,R,S)$."),
        ...
        "solutions": [
            "Balance the cross-flows: $.2 pi(S)=.4 pi(R)$, so $pi(S)=2 pi(R)$; normalizing gives $pi=(2/3,1/3)$.",
            "Initial probability times three transition probabilities: $0.6(.2)(.6)(.4)=.0288$.",
            "Maximizing $sum_j n_(ij) log P_(ij)$ with each row summing to one gives the count fractions $hat P_(ij)=n_(ij)/sum_j n_(ij)$.",
            "Add one to every count and renormalize: $(9,3,1,1)/14$ — unseen transitions get $1/14$ instead of an impossible zero.",
            "Each transition costs $-log_2$ of its probability: $1+2+3=6$ bits total, $6/3=2$ BPC, perplexity $2^2=4$.",
        ],
        "cells": [
...
def qmd_for(tutorial: dict) -> str:
    ...
    return (
        ...
        f'subtitle: "After {tutorial["after"].replace("L", "Lecture ")} · 80 minutes"\n'
        "---\n\n"
        f'{tutorial["intro"]}\n\n'
