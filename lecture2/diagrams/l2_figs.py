#!/usr/bin/env python3
"""Figures for L2 (Floating Point: How Machines See Numbers).
Run from repo root:  uv run --no-project --with matplotlib,numpy python3 lecture2/diagrams/l2_figs.py
Emits SVG + PNG twins (transparent, metropolis palette) into lecture2/figures/.
Figure bodies inherited from the Marp-era diagrams/lec02_figures.py; palette remapped.
"""
import os
import numpy as np
import matplotlib
...
# metropolis palette, keyed under the legacy names the figure bodies use
INK = "#23373B"; MUTED = "#6E7F82"; RULE = "#B9C4C6"
RUST = "#EB811B"; SAGE = "#2C7A7B"; SLATE = "#23373B"
WINE = "#D64550"; OCHRE = "#2B6CB0"; PAPER = "white"; PAPER_ALT = "#EFEEEB"
...
def save(fig, name, tight=True):
    ...
    fig.savefig(f"{OUT}/{name}.svg", format="svg", bbox_inches=bbox, transparent=True)
    fig.savefig(f"{OUT}/{name}.png", bbox_inches=bbox, transparent=True, dpi=200)
    plt.close(fig)
